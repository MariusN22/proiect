...
class PacientAlarmScreen extends StatefulWidget {
  const PacientAlarmScreen({Key? key}) : super(key: key);

  @override
  State<PacientAlarmScreen> createState() => _PacientAlarmScreenState();
}

class _PacientAlarmScreenState extends State<PacientAlarmScreen> {
  DateTime _selectedDay = DateTime.now();
  late String pacientId;

  @override
  void initState() {
    super.initState();
    pacientId = FirebaseAuth.instance.currentUser!.uid;
  }
  ...
  void _showEditAlarmBottomSheet(BuildContext context, DocumentSnapshot alarmDoc, Map<String, dynamic> data) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (alarms.isEmpty) {
          return Center(
              child: Text("Nu există alarme pentru această zi.",
                  style: TextStyle(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: Text(
                        widget.isEdit ? "Editează alarmă" : "Adaugă alarmă",
                        style: TextStyle(
                          color: themeGreen,
                          fontWeight: FontWeight.bold,
                          fontSize: 22,
                ...
                TextFormField(
                  ...
                  validator: (v) => v == null || v.isEmpty ? "Completează titlul" : null,
                ),
                    ...
                    Expanded(
                      child: ElevatedButton(
                        onPressed: isSaving
                          ...
                          try {
                            // === AICI CEREM PERMISIUNEA EXPLICIT ===
                            ...
                            if (widget.isEdit && widget.alarmDoc != null) {
                              ...
                              if (mounted) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text("Alarmă modificată!")),
                                );
                              }
                            } else {
                              ...
                              // ----- PROGRAMEZI NOTIFICAREA LOCALĂ:
                              final int notificationId = scheduledDate.millisecondsSinceEpoch.remainder(100000);
                              await scheduleAlarmNotification(
                                id: notificationId,
                                title: _titleController.text.trim(),
                                body: _detailsController.text.trim().isEmpty
                                    ? 'Este timpul să iei medicamentul!'
                              ...
                              if (mounted) {
                                ...
                                Navigator.of(context).pop();
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text("Alarmă salvată!")),
                                );
                              }
                            }
                          } catch (e) {
                        ...
                        child: isSaving
                            ...
                            : Text(widget.isEdit ? "Salvează modificarea" : "Salvează alarmă",
